...
#[AsCommand(
    name: 'iterm:open',
    description: '🚀 Open project in iTerm'
)]
final class ItermOpenCommand extends AbstractTerminalCommand
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->setHelp(
                <<<'HELP'
                    The <info>%command.name%</info> command opens a project in iTerm:
                    ...
                    Examples:
                    <info>php %command.full_name% my-project</info>    Open "my-project" in iTerm
                    <info>php %command.full_name% api-service</info>   Open "api-service" in iTerm
                    HELP
            );
    }
    ...
    /**
     * Open project in iTerm with proper error handling
     */
    private function openProjectInIterm(ProjectInterface $project, SymfonyStyle $io): int
    {
        ...
        try {
            ...
            if ($process->isSuccessful()) {
                $io->success(sprintf('Project "%s" opened in iTerm!', $project->getName()));
            ...
            $io->error(sprintf('Failed to open iTerm: %s', $e->getMessage()));
            return Command::FAILURE;
        }
    }

    /**
     * Generate AppleScript untuk buka tab baru di iTerm
     */
    ...
    private function openWithFallback(string $path, SymfonyStyle $io): int
    {
        try {
            ...
            $process = Process::fromShellCommandline(sprintf('open -a iTerm "%s"', $path));
            $process->run();
            ...
            $process = Process::fromShellCommandline(sprintf('cd "%s" && open -a iTerm .', $path));
            $process->run();

            if ($process->isSuccessful()) {
                $io->success('Opened current directory in iTerm!');
                return Command::SUCCESS;
            }

            $io->error('All methods failed. Please open iTerm manually.');
