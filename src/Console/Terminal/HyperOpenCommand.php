...
#[AsCommand(
    name: 'hyper:open',
    description: '🚀 Open project in Hyper'
)]
final class HyperOpenCommand extends AbstractTerminalCommand
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->setHelp(
                <<<'HELP'
                    The <info>%command.name%</info> command opens a project in Hyper:
                    ...
                    Examples:
                    <info>php %command.full_name% my-project</info>    Open "my-project" in Hyper
                    <info>php %command.full_name% api-service</info>   Open "api-service" in Hyper
                    HELP
            );
    }
    ...
    /**
     * Open project in Hyper with proper error handling
     */
    private function openProjectInIterm(ProjectInterface $project, SymfonyStyle $io): int
    {
        ...
        try {
            ...
            if ($process->isSuccessful()) {
                $io->success(sprintf('Project "%s" opened in Hyper!', $project->getName()));
            ...
            $io->error(sprintf('Failed to open Hyper: %s', $e->getMessage()));
            return Command::FAILURE;
        }
    }

    /**
     * Generate AppleScript untuk buka tab baru di Hyper
     */
    ...
    private function openWithFallback(string $path, SymfonyStyle $io): int
    {
        try {
            ...
            $process = Process::fromShellCommandline(sprintf('open -a Hyper "%s"', $path));
            $process->run();
            ...
            $process = Process::fromShellCommandline(sprintf('cd "%s" && open -a Hyper .', $path));
            $process->run();

            if ($process->isSuccessful()) {
                $io->success('Opened current directory in Hyper!');
                return Command::SUCCESS;
            }

            $io->error('All methods failed. Please open Hyper manually.');
